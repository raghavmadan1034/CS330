...
void *relocate_textseg(unsigned long start, unsigned long end)
{
     ...
     /*TODO your code goes in here*/
      reloc_addr=mmap(reloc_addr,end-start,PROT_READ|PROT_WRITE|PROT_EXEC,MAP_ANONYMOUS|MAP_PRIVATE,0,0);			
...
int main(int argc, char **argv)
{
   ...
   if(argc){ 
	  ...
	  */ 
	  ...
	  char* st=(char*)start_code;
	  char* ed=(char*)end_data;
	  char* temp=relocate_textseg(start_code,end_data);
	  reloc_code_addr=temp;
	  reloc_main=reloc_code_addr+((unsigned long)&main- start_code);
	  while(st<=ed){
		 *temp=*st;
		 temp++;
		 st++;
